...
function updateClientProfile (int $idclient, array $dataUpdate) // update client profile in the database (with client modification, on website) with $dataUpdate as an array
{
    $local_table = 'client';
...
function verifyExistingClientMail (string $mail) // verify in the database if the email address entered is already used or not
{
    $local_table = 'client';
...
function verifyExistingClientPseudo (string $pseudo) // verify in the database if the pseudo entered is already used or not
{
    $local_table = 'client';
...
function connexionTry() // try to connect a client
{
    $local_table = 'client';
...
function cancelSub(int $idclient) // cancel the subscription of the client, not regarding to the offer
{
    $local_table = 'client';
...
function setSub($idclient, $sublevel) // set client subscription
{
    $local_table = 'client';
...
function tryChangePassword() // try to change client password
{
    $local_table = 'client';
...
function checkClientConnexion() // check if the client is connected
{
    if (!isset($_COOKIE['is_connected']) or $_COOKIE['is_connected'] == false){
        header('Location: '. get_url('connexion','index')); // redirect on the view to connexion index (expect a client connexion)
        exit;
    } else {
...
function checkClientSub() // check if the client has a subscription
{
    if ($_SESSION['Sub'] == null){
        $error_message = "Erreur : Vous ne pouvez pas accéder aux pages d'entraînements.<br><br> Raison : Vous n'avez pas d'abonnement";
        $local_data = ['error_message' => $error_message];
        display_view('pagemembre/index', $local_data); // redirect on connexion page
        exit;
    }
}
